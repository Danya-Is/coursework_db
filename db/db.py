...
def execute_one_query(query: str):
...
def execute_all_query(query: str):
    with get_db_connection().cursor() as cursor:
        cursor.execute(query)
...
def get_by_unique_str(table: str, by: str, value: str) -> Union[Tuple, None]:
    return execute_one_query('SELECT * FROM {} WHERE {}=\'{}\''.format(table, by, value))
...
def get_all(table: str):
    return execute_all_query('SELECT * FROM {}'.format(table))
...
def get_available_films():
    ...
    return execute_all_query(queue)
...
def get_film_stat(film_id: int) -> float:
    sessions = execute_all_query('SELECT * FROM cinema_session as s WHERE s.film_id={}'
                                 .format(film_id))
    sum = 0
...
def get_genre_stat(genre_id: int) -> float:
    films = execute_all_query('SELECT * FROM film as f WHERE'
